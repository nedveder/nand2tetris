...
class CodeWriter:
    ...
    def write_bootstrap(self) -> None:
        ...
        self.write_code(code+["(COMP_INIT)"])
    ...
    def write_push_pop(self, command: str, segment: str, index: int) -> None:
        ...
        memory_segments: dict[str, list[str]] = {"local": ["@1", "D=M", f"@{index}", "A=A+D"],
                                                 ...
                                                 "pointer": [f"@{3+index}"]}
        code: list[str] = [
            f"//PUSH/POP {command} segment:{segment} index:{index}"]
        code += memory_segments[segment]+["D=A", "@R14", "M=D"]
        if command == "C_PUSH":
            reg: list[str] = [
                "D=M"] if segment == "constant" else ["A=M", "D=M"]
            code += ["@R14"]+reg
            code += STACK_PUSH
        elif command == "C_POP":
            code += STACK_POP
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        ...
        if function_name not in self._call_dict:
            self._call_dict[function_name] = 0
        return_address: str = f"{function_name}$ret{self._call_dict[function_name]}"
        code += [f"@{return_address}", "D=A"]+STACK_PUSH
        self._call_dict[function_name] += 1
