...
# ["@SP", "M=M-1", "@SP", "A=M", "D=M"]
STACK_POP = ["@SP", "AM=M-1", "D=M"]
# ["@SP", "A=M", "M=D", "@SP", "M=M+1"]
STACK_PUSH = ["@SP", "M=M+1", "A=M-1", "M=D"]
...
class CodeWriter:
    ...
    def write_push_pop(self, command: str, segment: str, index: int) -> None:
        ...
        memory_segments = {"local": ["@1", "D=M", f"@{index}", "A=A+D"],
                           ...
                           "pointer": [f"@{3+index}"]}
        code = [f"//PUSH/POP {command} segment:{segment} index:{index}"]
        code += memory_segments[segment]+["D=A", "@R14", "M=D"]
        if command == "C_PUSH":
            reg = ["D=M"] if segment == "constant" else ["A=M", "D=M"]
            code += ["@R14"]+reg
            code += STACK_PUSH
        elif command == "C_POP":
            code += STACK_POP
    ...
    def write_function(self, function_name: str, n_vars: int) -> None:
        ...
        """
        code = [f"// FUNCTION {function_name} vars:{n_vars}"]
    ...
    def write_call(self, function_name: str, n_args: int) -> None:
        ...
        """
        code = [f"// CALL {function_name} vars:{n_args}"]
        # Push return address into stack
        if function_name not in self._call_dict:
            self._call_dict[function_name] = 0
        return_address = f"{self._function_name}$ret{self._call_dict[self._function_name]}"
    ...
    def write_return(self) -> None:
        ...
        code = [f"// RETURN {self._function_name}"]
        # Put frame(R14) = LCL
        code += ["@LCL", "D=M", "@R14", "M=D"]
        # Put retAddress(R15) = frame - 5
        code += [f"@{5}", "A=D-A", "D=M", "@R15", "M=D"]
        # Pop stack into ARG
